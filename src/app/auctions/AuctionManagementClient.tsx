...
export default function AuctionManagementClient() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      } catch (err) { setError(err instanceof Error ? err.message : "Unable to load auctions."); }
    })();
  }, [loading, router, user]);
  ...
  function selectAuction(auction: Auction) {
    setSelected(auction);
    setForm({ name: auction.name, description: auction.description, status: auction.status, mode: auction.settings.mode, currency: auction.settings.currency, timezone: auction.settings.timezone, startAt: dateValue(auction.startAt), endAt: dateValue(auction.endAt) });
    setTheme(mergeTheme(auction.settings.theme)); setError(""); setNotice("");
  }

  async function save() {
    if (!user || !form.name.trim()) { setError("Auction name is required."); return; }
    ...
    try {
      ...
      setAuctions(await listAuctionsForUser(user));
    } catch (err) { setError(err instanceof Error ? err.message : "Unable to save auction."); }
  ...
  async function changeStatus(status: AuctionStatus) {
    ...
    try {
      ...
      setSelected(refreshed); setForm((current) => ({ ...current, status }));
  ...
  async function remove() {
    ...
    setBusy(true);
    try { await deleteAuction(selected.id); setAuctions((current) => current.filter((item) => item.id !== selected.id)); startCreate(); setNotice("Auction deleted."); }
    ...
    <main className={styles.shell}>
      ...
      <div className={styles.layout}>
        <aside className={styles.listPanel}>
          ...
          {sortedAuctions.length === 0 ? <div className={styles.empty}>No auctions yet.<br />Create your first league auction.</div> : sortedAuctions.map((auction) => (
            <button key={auction.id} className={`${styles.auctionItem} ${selected?.id === auction.id ? styles.selected : ""}`} onClick={() => selectAuction(auction)}>
              <span className={styles.itemTop}><strong>{auction.name}</strong><em>{auction.status}</em></span>
