...
import {
  ...
  listAuctionsForUser,
  updateAuction,
} from "@/lib/auctions/repository";
import {
  AUCTION_MODES,
  AUCTION_STATUSES,
  DEFAULT_AUCTION_THEME,
  type Auction,
  type AuctionStatus,
  type AuctionTheme,
...
function mergeTheme(theme?: Partial<AuctionTheme>): AuctionTheme {
  return { ...DEFAULT_AUCTION_THEME, ...theme };
}
...
export default function AuctionsPage() {
  ...
  const sortedAuctions = useMemo(
    () => [...auctions].sort((a, b) => b.updatedAt.toMillis() - a.updatedAt.toMillis()),
    [auctions],
  );
  ...
  function selectAuction(auction: Auction) {
    ...
    setForm({
      ...
      mode: auction.settings.mode,
      currency: auction.settings.currency,
      timezone: auction.settings.timezone,
      startAt: dateValue(auction.startAt),
      endAt: dateValue(auction.endAt),
    });
  ...
  async function save() {
    ...
    try {
      if (selected) {
        await updateAuction(selected.id, {
          name: form.name.trim(),
          description: form.description.trim(),
          status: form.status,
          startAt: timestampValue(form.startAt),
          endAt: timestampValue(form.endAt),
          settings: {
            mode: form.mode,
      ...
      setError(err instanceof Error ? err.message : "Unable to save auction.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function changeStatus(status: AuctionStatus) {
    ...
    try {
      ...
      setAuctions((current) => current.map((item) => item.id === selected.id ? refreshed : item));
      setNotice(`Auction moved to ${status}.`);
    } catch (err) {
  ...
  if (loading || !user) {
    return <main className={styles.loading}>Loading auction management…</main>;
  }
  ...
  return (
    <main className={styles.shell}>
      <header className={styles.header}>
        <div>
          <p className={styles.eyebrow}>Auction Platform / Management</p>
          <h1>Auction Management</h1>
          <p className={styles.subtitle}>Create, configure, schedule and govern every league auction.</p>
        </div>
        <div className={styles.headerActions}>
          <span className={styles.role}>{isSuperAdmin ? "SUPER ADMIN" : "AUCTION ADMIN"}</span>
      ...
      <div className={styles.layout}>
        <aside className={styles.listPanel}>
          ...
          {sortedAuctions.length === 0 ? (
            <div className={styles.empty}>No auctions yet.<br />Create your first league auction.</div>
          ) : sortedAuctions.map((auction) => (
            <button
              key={auction.id}
              className={`${styles.auctionItem} ${selected?.id === auction.id ? styles.selected : ""}`}
              onClick={() => selectAuction(auction)}
            >
              <span className={styles.itemTop}><strong>{auction.name}</strong><em data-status={auction.status}>{auction.status}</em></span>
              <span className={styles.itemBottom}>{auction.settings.mode} · {auction.settings.currency} · {auction.settings.timezone}</span>
            </button>
          ))}
        </aside>
        ...
        <section className={styles.editor}>
          <div className={styles.sectionTitle}>
            <div><p className={styles.eyebrow}>{selected ? "Edit auction" : "New auction"}</p><h2>{selected?.name || "Auction configuration"}</h2></div>
            {selected && <span className={styles.id}>ID {selected.id}</span>}
          </div>
          ...
          <div className={styles.formGrid}>
            <label>AUCTION NAME<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Premier League 2026" /></label>
          ...
          <div className={styles.themePanel}>
            <div><p className={styles.eyebrow}>Auction-level visual identity</p><h3>Theme & branding</h3><p>Each auction owns its presentation. Defaults are Luxury Dark; changes here never alter another auction.</p></div>
            <div className={styles.themeGrid}>
              <label>MODE<select value={theme.mode} onChange={(e) => setTheme({ ...theme, mode: e.target.value as "dark" | "light" })}><option value="dark">Dark luxury</option><option value="light">Light gallery</option></select></label>
          ...
          <div className={styles.footerActions}>
            ...
            {selected && <div className={styles.lifecycle}>{(["draft", "scheduled", "live", "paused", "completed", "archived"] as AuctionStatus[]).map((status) => <button key={status} className={form.status === status ? styles.statusActive : styles.statusButton} disabled={busy} onClick={() => void changeStatus(status)}>{status}</button>)}</div>}
