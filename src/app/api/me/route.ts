...
async function authorize(request: Request) {
  ...
  const decoded = await getAuth(app).verifyIdToken(header.slice("Bearer ".length), true);
  return { adminAuth: getAuth(app), db: getFirestore(app), uid: decoded.uid, decoded };
}
...
export async function GET(request: Request) {
  try {
    const { adminAuth, db, uid, decoded } = await authorize(request);
    const [authUser, profile] = await Promise.all([adminAuth.getUser(uid), db.collection("users").doc(uid).get()]);
    const data = profile.data() ?? {};
    return NextResponse.json({
      uid,
      name: authUser.displayName ?? data.displayName ?? "",
      email: authUser.email ?? data.email ?? "",
      photoUrl: authUser.photoURL ?? data.photoUrl ?? null,
      expertise: data.expertise ?? "",
      role: decoded.superAdmin === true ? "superAdmin" : data.role ?? "viewer",
...
export async function PATCH(request: Request) {
  try {
    const { adminAuth, db, uid } = await authorize(request);
