...
test("full auction flow provisions roles, configures an auction, bids and settles a lot", async ({ page, browser }) => {
  ...
  console.log("E2E_CREATE_AUCTION_RESPONSE", createResponse.status(), JSON.stringify(createdAuction));
  expect(createResponse.ok()).toBeTruthy();
  expect(createdAuction.id).toBeTruthy();
  await expect(superAdminPage).toHaveURL(new RegExp(`/auctions/${createdAuction.id}/* eslint-disable @typescript-eslint/no-require-imports */
const { test, expect } = require("@playwright/test");
